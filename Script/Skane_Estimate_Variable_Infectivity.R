...
#------------------------------------------------------------------------------------
#
# FILE:         Skane_Estimate_Variable_Infectivity
...
#                       The main focus of this script is the region of Skane, 
...
Estimate_function_region_1R <- function(Region, 
  ...
  GuessesT <- function(){ # guesses for the optimisation
    ...
    #transformed 
    u_p <- logit(runif(1, 0, 0.6)) #
    u_e <- runif(1, -3, 0) #
    u_t <- log(runif(1, 0, 15)) #
    if(p_lower_inf > 0.5){ u_t <- log(runif(1, 0, 1)) }
...
dayatyear_march_september <- c(First_feb, First_march, 92, 122, 
                               153, 183 , 214, 245,275)
...
pdf(paste(figure.path, NameNumber, sep=""), width=7.5, height=6 )
  ...
  plot(fit$time, fit_cum_inf[1:length(t)]/N, type="l", col="black", ylab = "E + I + R" , lwd = 2,
       xlab="", 
       # ylim=c(0, roundUpNice(max(fit_cum_high[1:length(t)]/N) + 0.05)), 
       ylim=c(0, 0.3), 
...
# here, 10%, 20%, 30%, 40%, and 50%
Increase_contact <- seq(1.1, 1.5, by = 0.1)
...
for(i in 1:length(Increase_contact)){
  ...
  if(SEED_W_ONE==TRUE){
    init <- c(S = N - Observed_incidence[1], 
              E = 0, 
              I_symp = Observed_incidence[1], 
              I_asymp = 0, 
              R = 0)
  }else{
    init <- c(S = N - Observed_incidence[1]*(1 + (1-p_symp_use)/p_symp_use), 
              E = 0, I_symp = Observed_incidence[1], 
              I_asymp = Observed_incidence[1]*(1-p_symp_use)/p_symp_use,
              R = 0)
  }
