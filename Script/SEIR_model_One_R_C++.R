...
class SEIR {
public:
  double p, epsilon, theta, t_today, N, p_lower_inf, eta, p_symp, gammaD, t_start, t_end;
  SEIR(double p, double epsilon, double theta, double t_today, double N, double p_lower_inf,
       double eta, double p_symp, double gammaD, double t_start, double t_end) : p(p), epsilon(epsilon), theta(theta),
						   t_today(t_today), N(N), p_lower_inf(p_lower_inf),
						   eta(eta), p_symp(p_symp), gammaD(gammaD), t_start(t_start), t_end(t_end) { }
  double b(double t) {
...
arma::mat SEIRmodel2(Rcpp::NumericVector state, Rcpp::NumericVector parms) {
  ...
  arma::vec init(&state[0], 5);
  SEIR model(p, epsilon, theta, t_today, N, p_lower_inf, eta, p_symp, gammaD, t_start, t_end);
