...
#------------------------------------------------------------------------------------
#
# FILE:         Dalarna_Estimate_Variable_Infectivity
...
#                       in the Stockholm region, Vastra Gotaland region, Skane region, and Dalarna region, Sweden.'
#                       The main focus of this script is the region of Dalarna, 
#                       and it produces the main results for this region. 
#                       
#                       
#                   
#                   
...
########################################################################
#                                                                      
# For the other regions than Stockholm, we need to use the estimated fraction of cases 
# that are reported based on Stockholm data as a known constant.              
# We therefore do not need the model with two recovered compartments for the other regions than Stockholm                                                                           
#
########################################################################
...
# 3. Libraries
# 4. Read incidence data 
# 5. Read Population data
# 6. Choice of parameter values and region
# 7. Basic functions and set.up
# 8. Main estimation function
# 9. Estimation
# 10. Initial inspection of results
# 11. Calculate results to save in tables. 
...
#  12. Plot figures
#  13. Analysing increased contacts
...
## R := Recovered/immune compartment. 
##
####
...
## For the other regions than Stockholm, the below values is kept as a remainder of the assumption on 
## testing window for the PCR-test. It is not used in analysis.
...
# With the 10 day testing window the fraction of unreported cases was estimated to 0.9821
p_symp_use      <- 1 - 0.9821 #p_symp estimated based on Stockholm data
...
# read model (with one R compartment) and ODE function 
source(paste(project.path,"Script/SEIR_model_One_R_C++.R",sep=""))
...
#   - Opt_par: The estimated parameters, p, theta, and epsilon
#     p, theta, and epsilon are the parameters governing the time-dependent infectivity 
#
#   - SEIR_model: the model that calculates the number of individuals in each compartments at different 
#                 time points
#
#   - Initial_values: The initial values for SEIR_model, if seed_w_one = FALSE. 
#
...
Estimate_function_region_1R <- function(Region, 
                                        p_symp = p_symp, 
                                        p_lower_inf = 1, 
                                        gammaD = gammaD1, 
                                        eta = eta1, 
  ...
  N <- unname(N_region[Region])
  
  if(Region == "Dalarna"){

    Fixed_data <- Dalarna_Data
    
  }
  if(Region == "Skane"){
    
    Fixed_data <- Skane_Data
    
  }
  if(Region == "Vastra Gotaland"){
    
    Fixed_data <- VGR_Data
    
  }
  
  Incidence <- Fixed_data$Incidens
  Datum     <- as.Date(Fixed_data$Datum)
  ...
  Opt_par_names <- c("p","epsilon","theta")
  Opt_par_names_t <- c("logit_p", "epsilon", "log_theta")  
  ...
  GuessesT <- function(){ # guesses for the optimisation
    ...
    u_e <- runif(1,-0.6, 0)    #
    u_t <-  log(runif(1, 0, 15))    #
    if(p_lower_inf > 0.5){ u_t <- log(runif(1, 0, 2)) }
    
    
    return(c(u_p, u_e, u_t))
  ...
  Basic_repr<- function(t,p,epsilon,theta,gamma){ 
  ...
  seir.model.asymptomatics <- function(time, state, parameters) {
    t_today <- as.numeric(as.Date("2020-03-16")) - as.numeric(as.Date("2019-12-31")) # dagen för jobba hemma
    beta.peak.free <- function(t,  p, epsilon,theta){
      ((1-p)/(1+exp(epsilon*(-(t-t_today)))) + p)* theta
    }
  ...
  RSST <- function(parameters) {
    ...
    Dummy_infectivity <- beta.peak.free(t = c(0: 700), p = expit(parameters[1]), epsilon = parameters[2],  theta = exp(parameters[3]))
    ...
    if(min(Dummy_infectivity) < 0 ){
      ...
      #out <- ode(y = init, times = Day, func = SEIRmodel, parms = parms)
      ...
      LogL <- -length(Incidence) * log(sum((Incidence - fitted_incidence)^2)) / 2
      return(-LogL)
    }
  }
  ...
  Guess <- GuessesT()
  Opt <- 0
  conl <- list(maxit = 1000, abstol = 1e-8, reltol = 1e-10) # decreasing
  
  Opt <- optim(Guess, RSST, control = list(conl), hessian = TRUE)
  Opt 
  ...
  while(i <= iter){
    ...
    Opt2 <- optim(Guess, RSST, control = list(conl), hessian = TRUE)
  ...
  return(list(Observed_incidence = Incidence, Population_size = N, Day = Day, dayatyear = dayatyear, Namedate = Namedate, Optimisation = Opt, 
              Opt_par = Opt_par, Seasonal_forcing = beta.peak.free, Basic_reproduction = Basic_repr, Initial_values = init, SEIR_model = model))
...
Est_par_model <- Estimate_function_region_1R(Region = REGION, 
                                             iter = 20, 
                                             p_symp = p_symp_use, 
...
t <- (Day[1]):(Day[length(Day)]+260) # time in days
...
#pdf(paste(figure.path, NameNumber, sep=""), width=9, height=5 )
  ...
  plot(Day,Observed_incidence,type="o", ylab = "Reported cases", xlab ="",xlim =c(Day[1],365), xaxt ='n',
...
t_Re   <- c(Day[1]:200)
Re     <- Basic_repr(t_Re, p = Opt_par[1], epsilon = Opt_par[2],  theta = Opt_par[3], gamma = gammaD) * fit_S[t_Re] / N
...
#pdf(paste(figure.path, NameNumber, sep=""), width=9*1.25, height=4.5*1.3 )
  ...
  if(roundUpNice(max(Re)) >=4 ){
    abline(h = c(0:20), col = "lightgray", lty = "dotted", lwd = par("lwd"))
  }else{
    abline(h = seq(0,5,by=0.5), col = "lightgray", lty = "dotted", lwd = par("lwd"))
  }
  #
  ...
  if(roundUpNice(max(infect)) >= 4 ){
    abline(h = c(0:20), col = "lightgray", lty = "dotted", lwd = par("lwd"))
  }else if(roundUpNice(max(infect)) >= 1.5 ){
    abline(h = seq(0,5,by=0.5), col = "lightgray", lty = "dotted", lwd = par("lwd"))
  }else{
    abline(h = seq(0,5,by=0.2), col = "lightgray", lty = "dotted", lwd = par("lwd"))
  }
...
R0_Mean <- Basic_repr(Day, p = Opt_par[1], epsilon = Opt_par[2],  theta = Opt_par[3]  ,gamma = gammaD)
...
res_param <- c(round(p_asymp_use, digits = 4), 
               round(c(LogL_MLE, RSS_value, Opt_par[1], sdParams[1], Opt_par[2], sdParams[2], Opt_par[3], sdParams[3]), digits = 3))

names(res_param) <- c("p_o: fraction unreported", "LogL","RSS" ,"delta", "logit s.e.", "epsilon", "s.e.", "theta", "log s.e.")
...
MAT_para <- matrix(c(res_param,CI_param),ncol = 9, nrow = 2, byrow = TRUE)
...
for(i in 1:length(p.v)){
  ...
  fitted_incidence_nonreport.v[i,]  <- (1 - p_symp_use) *  fitDummy[ , 3] * eta
  fitted_incidence_tot.v[i,]        <- fitDummy[ , 3] * eta
  effective_reprod.v[i,] <- Basic_repr(fitDummy[,1], p = Opt_parDummy[1], epsilon = Opt_parDummy[2], theta = Opt_parDummy[3], gamma = gammaD) * fitDummy[ , 2] /N
}
...
res_R0        <- round(c(p_asymp_use, infect[1], infect[which(t_Re == Day[length(Day)])], contact_reduced, 
                         infect[which(t_Re == 162)],
                         R0_Mean[1], R0_Mean[length(Day)],effective_reprod_mean[length(Day)]), digits = 3)
names(res_R0) <- c("p_0", "infectivity(start)","infectivity(end)","infectivity reduced", "infectivity 10/6", "R0(start)", "R0(end)", "Re(end)")
...
MAT_R0    <- matrix(c(res_R0,CIR0), ncol = 8, nrow = 2, byrow = TRUE)
...
pdf(paste(figure.path, NameNumber, sep=""), width=7.5, height=6 )
  ...
  plot(W14_to_W22, fit_antibodies[W14_to_W22]/N, type="l", col="black", ylab = "I + R" ,lwd=2,
       xlab="",ylim=c(0,roundUpNice(max(fit_antibodies_95_up_CRI[W14_to_W22]/N))), 
...
dayatyear_march_september <- c(First_march, 92, 122, 
                               153, 183 , 214, 245,275)
...
pdf(paste(figure.path, NameNumber, sep=""), width=9*1.5, height=4.5*1.3 )
  ...
  plot(fit$time, fitted_incidence, type = "l", col = "red", ylab = "Incidence", main = "" , lwd = 2,
       xlab = "", 
       ylim = c(0, roundUpNice( max(fitted_incidence_high, Observed_incidence) + 1)), 
       xlim = range(dayatyear_march_september), 
       xaxt='n')
  ...
  plot(fit$time, fit[ , 4]+fit[ , 5], 
       ylim = c(0, roundUpNice( max(fitted_I_high) + 10) ) ,
       xlim = range(dayatyear_march_september),
       type = "l", col = "red", 
...
## no contact increase as baseline
t <- c(Day[1]:450)
fit <- data.frame(ode(y = init, times = t, func = SEIR_model , parms = Opt_par))
fit_S <- fit[ , 2]
fit_E <- fit[ , 3]
...
fitted_incidence_reported_same  <- p_symp_use * fit_E * eta
fitted_incidence_non_reported_same  <- (1 - p_symp_use) * fit_E * eta
...
for(i in 1:length(Increase_contact)){
  ...
  seir.model.asymptomatics <- function(time, state, parameters) {
    ...
    with(par, {
      ...
      dI_symp <- p_symp_use * eta * E        - gammaD * I_symp
      dI_asymp <- (1 - p_symp_use) * eta * E - gammaD * I_asymp
      dR <- gammaD * (I_symp + I_asymp)
      dx <- c(dS, dE, dI_symp, dI_asymp, dR)
      list(dx)
    }
  ...
  if(SEED_W_ONE==TRUE){
    init <- c(S = N - Observed_incidence[1], E = 0, I_symp = Observed_incidence[1], I_asymp = 0 , R = 0)
  }else{
    init <- c(S = N - Observed_incidence[1]*(1 + (1-p_symp_use)/p_symp_use), E = 0, I_symp = Observed_incidence[1], I_asymp = Observed_incidence[1]*(1-p_symp_use)/p_symp_use , R = 0)
  }
  ...
  fit_E <- fit[ , 3]
  fitted_incidence_reported_inc_contact[[i]]  <- p_symp_use * fit_E * eta
  fitted_incidence_non_reported_inc_contact[[i]]  <- (1 - p_symp_use) * fit_E * eta
  ...
  Incidence_df <- data.frame(time = fit[,1], 
                             ...
                             Incidence = fit_E * eta, 
                             Incidence_report = p_symp_use * fit_E * eta, 
                             Incidence_nonreport = (1 - p_symp_use) * fit_E * eta)
...
dayatyear_feb_jan <- c(First_feb, First_march, 92, 122, 
                       153, 183 , 214, 245,275, 306,336, 367, 398, 426, 457)
...
yMax <- roundUpNice(max( fitted_incidence_reported_inc_contact[[length(Increase_contact)]],Observed_incidence))
plot(Day,Observed_incidence, type = "p", ylab = "Reported cases", xlab ="", 
     #xlim = range(dayatyear_feb_jan), 
     xlim = c(First_feb, t[length(t)]),
     xaxt ='n', 
     main = paste("Estimated and simulated number of reported cases: ", REGION, sep=""),
     ylim = c(0, yMax))
...
legend(t[length(t)]+30,yMax, c("Observed reported cases", "Fitted cases no change", Legend_Name), 
